...
| 顺序 | 工作项 | 预期价值 | 精度要求 |
| --- | --- | --- | --- |
| P0 | 验证 BigVGAN 备用卷积后端并调整选择策略 | 已有明确阶段速度证据，潜在收益最大 | 原要求保持 dtype；实际备用路径转换为 FP16 权重，见第 4 节核查更正 |
...
**实现核查更正（用户询问 MPS 加速原因时发现）：** 原 `mit2_conv1d_dilated_same_f32` 使用 FP32 权重、输入和输出；备用路径通过 `resident_buffer_f16_from_f32()` 将按卷积核位置重排的权重转换为 FP16，MPS 的输入/输出矩阵描述仍为 FP32。因而 P0 跨后端对比同时改变了计算组织和权重表示，实际并未满足原计划的“权重 dtype 不变”要求。此前把波形差异只解释为浮点累加顺序变化不完整，也不能把全部提速归因于 MPS 内核。尚未进行同权重精度的独立对照，无法分别量化这些因素的贡献；用户已接受的本句听感和实际耗时记录保留，但它们不证明数值无损。
...
- 原要求不改变权重与激活 dtype；现已核查备用路径会将这部分权重转为 FP16，属于未满足原约束的既有实现，不能标为保持权重精度的优化。输入/输出矩阵仍为 FP32。
