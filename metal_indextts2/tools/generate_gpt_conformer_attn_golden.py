...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with torch.no_grad():
        with torch.amp.autocast(tts.device, enabled=tts.dtype is not None, dtype=tts.dtype):
            ...
            normed = layer.norm_mha(subsampled)
            attn_out, _cache = layer.self_attn(normed, normed, normed, out_mask, pos_emb)
            residual = subsampled + attn_out
    ...
    attn_np = _as_numpy(attn_out.squeeze(0), np.float32)
    residual_np = _as_numpy(residual.squeeze(0), np.float32)

    manifest = {
        "format": "mit2-gpt-conformer-attn-golden",
        "version": 1,
        ...
        "heads": int(layer.self_attn.h),
        "head_dim": int(layer.self_attn.d_k),
        "tensors": {
            "attn_input": _write_raw(out_dir / "attn_input.f32", input_np),
            "pos_emb": _write_raw(out_dir / "pos_emb.f32", pos_np),
            "attn_mask": _write_raw(out_dir / "attn_mask.u32", mask_np),
            "attn_normed": _write_raw(out_dir / "attn_normed.f32", normed_np),
            "attn_out": _write_raw(out_dir / "attn_out.f32", attn_np),
            "attn_residual": _write_raw(out_dir / "attn_residual.f32", residual_np),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a GPT conditioning Conformer relative attention golden fixture.")
