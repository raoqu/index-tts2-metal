...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with torch.no_grad():
        with torch.amp.autocast(tts.device, enabled=tts.dtype is not None, dtype=tts.dtype):
            ...
            attn_normed = layer.norm_mha(subsampled)
            attn_out, _cache = layer.self_attn(attn_normed, attn_normed, attn_normed, out_mask, pos_emb)
            conv_input = subsampled + attn_out
            conv_normed = layer.norm_conv(conv_input)
            conv_raw, _cnn_cache = layer.conv_module(conv_normed, out_mask)
            conv_residual = conv_input + conv_raw

    input_np = _as_numpy(conv_input.squeeze(0), np.float32)
    mask_np = _as_numpy(out_mask.squeeze(0).squeeze(0), np.uint32)
    normed_np = _as_numpy(conv_normed.squeeze(0), np.float32)
    raw_np = _as_numpy(conv_raw.squeeze(0), np.float32)
    residual_np = _as_numpy(conv_residual.squeeze(0), np.float32)

    manifest = {
        "format": "mit2-gpt-conformer-conv-golden",
        "version": 1,
        ...
        "kernel_size": int(layer.conv_module.depthwise_conv.kernel_size[0]),
        "tensors": {
            "conv_input": _write_raw(out_dir / "conv_input.f32", input_np),
            "conv_mask": _write_raw(out_dir / "conv_mask.u32", mask_np),
            "conv_normed": _write_raw(out_dir / "conv_normed.f32", normed_np),
            "conv_raw": _write_raw(out_dir / "conv_raw.f32", raw_np),
            "conv_residual": _write_raw(out_dir / "conv_residual.f32", residual_np),
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a GPT conditioning Conformer convolution golden fixture.")
