...
std::vector<RemoteFile> fetchFileList(const RepoSpec& spec, long timeout_ms) {
    ...
    files.erase(std::unique(files.begin(), files.end(), [](const RemoteFile& lhs, const RemoteFile& rhs) {
                ...
                files.end());
    if (files.empty()) {
        throw std::runtime_error("no downloadable files found for " + providerName(spec.type) + ":" + spec.repo_id);
    }
    return files;
}
...
std::vector<Candidate> loadCandidates(const std::vector<RepoSpec>& specs) {
    ...
    if (specs.size() == 1) {
        std::cerr << "Listing " << providerName(specs.front().type) << ":" << specs.front().repo_id << "...\n";
...
void downloadOneFile(const RepoSpec& spec, const RemoteFile& remote_file, const fs::path& output_dir, std::size_t index, std::size_t count, bool force_update) {
    ...
    if (code != CURLE_OK) {
        throw std::runtime_error("download failed for " + remote_file.path + ": " + curlError(code, error_buffer));
    }

    const std::uintmax_t downloaded = fileSizeOrZero(part_path);
    if (expected_size > 0 && downloaded != expected_size) {
        throw std::runtime_error("downloaded size mismatch for " + remote_file.path + ": got " +
    ...
    if (!remote_file.sha256.empty()) {
        ...
        if (actual_hash != remote_file.sha256) {
            throw std::runtime_error("sha256 mismatch for " + remote_file.path + ": got " + actual_hash +
...
void DownloadModel(const std::vector<std::string>& urls, const fs::path& target_path, bool force_update) {
    ...
    for (std::size_t i = 0; i < selected.files.size(); ++i) {
        downloadOneFile(selected.spec, selected.files[i], output_dir, i + 1, selected.files.size(), force_update);
    }
