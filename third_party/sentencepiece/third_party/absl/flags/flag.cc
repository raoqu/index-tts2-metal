...
// mit2 vendor patch: renamed "help" -> "spm_help" to avoid a duplicate global
// `FLAGS_help` symbol clashing with OpenFST's flags.cc when both are linked
// statically into mtts. Only sentencepiece's (unbuilt) CLI tools used it.
ABSL_FLAG(bool, spm_help, false, "show help");
...
std::vector<char *> ParseCommandLine(int argc, char *argv[]) {
  ...
  if (absl::GetFlag(FLAGS_spm_help)) {
